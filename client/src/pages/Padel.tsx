...
export default function Padel() {
  ...
  // State för att hantera "Visa mer" 
  ...
  // Funktion för att toggla detaljer
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>, index: number) => {
    ...
    emailjs.send(
      ...
      alert('Något gick fel vid skickandet – prova igen.');
    })
  }
    ...
    <>
      <div className="relative bottom-20 h-[25vh] md:h-[35vh] overflow-hidden">
        ...
        {data?.videoUrl ? (
          ...
          <img src={urlFor(data.heroImage).width(1920).url()} alt="Padel" className="absolute top-0 left-0 w-full h-full object-cover" />
        ) : null}
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-12 pt-0 pb-10">
          <div className="prose prose-lg mx-auto text-center">
            <PortableText
              ...
              components={{
                block: {
                  h1: ({ children }) => <h1 className="text-4xl md:text-5xl font-black text-primary mt-0 mb-8 tracking-tight">{children}</h1>,
                  h2: ({ children }) => <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">{children}</h2>,
                  normal: ({ children }) => <p className="text-base md:text-lg leading-relaxed text-gray-700 mt-0 mb-4 font-light">{children}</p>,
                },
                marks: {
        ...
        {hasAktiviteter ? (
                ...
                return (
                  <div key={i} className="rounded-xl border border-gray-200 bg-white shadow-lg flex flex-col transition-all duration-300">
                    ...
                    <div className="p-6 flex-grow">
                      <h3 className="text-2xl font-bold text-gray-900 mb-3">{akt.aktivitet}</h3>
                      ...
                      {sentForms.includes(i) ? (
                        ...
                        <form onSubmit={(e) => handleSubmit(e, i)} className="space-y-8">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <input type="text" name="name" placeholder="Namn*" required className="w-full px-5 py-3 border-2 border-gray-300 rounded-lg bg-white placeholder-gray-500 transition duration-300 focus:border-primary focus:ring-4 focus:ring-primary/20 focus:outline-none text-lg shadow-sm" />
                            <input type="email" name="email" placeholder="E-post*" required className="w-full px-5 py-3 border-2 border-gray-300 rounded-lg bg-white placeholder-gray-500 transition duration-300 focus:border-primary focus:ring-4 focus:ring-primary/20 focus:outline-none text-lg shadow-sm" />
                          </div>
                          <input type="tel" name="phone" placeholder="Telefon (valfritt)" className="w-full px-5 py-3 border-2 border-gray-300 rounded-lg bg-white placeholder-gray-500 transition duration-300 focus:border-primary focus:ring-4 focus:ring-primary/20 focus:outline-none text-lg shadow-sm" />
                          <textarea name="message" placeholder="Meddelande / Önskemål (valfritt)" rows={5} className="w-full px-5 py-3 border-2 border-gray-300 rounded-lg bg-white placeholder-gray-500 transition duration-300 focus:border-primary focus:ring-4 focus:ring-primary/20 focus:outline-none text-lg resize-none shadow-sm" />
          ...
          <div className="max-w-5xl mx-auto px-4 py-0 text-center">
            <div className="bg-white shadow-lg p-12 max-w-3xl mx-auto border-t-4 border-gray-200">
              ...
              <h2 className="text-4xl font-bold text-gray-900 mb-4">Inga aktiviteter just nu</h2>
              <p className="text-xl text-gray-600 mb-8">Håll utkik – nya aktiviteter läggs upp löpande!</p>
