...
export default function Padel() {
  ...
  useEffect(() => {
    Promise.all([
      client.fetch(`*[_type == "padelaktivitet"][0]{
        ...
        content,
        aktiviteter[]{ "titel": aktivitet, info, anmalanTyp, url, formText, detaljer }
      }`),
  ...
  if (!data) return <Spinner label="Hämtar padelinformation…" />
  ...
  return (
    <>
      <PageHero videoUrl={data.videoUrl} image={data.heroImage} title={data.heroTitle || 'Padel'} />
